...
- (void)getSelectedAddressSourceWithRegionArray:(NSArray<BMNewRegionModel *> *)regionArray {
    ...
    switch (self.dataSourceType) {
        case BMAddressDataSourceTypeDB: {
            ...
            [arrayM enumerateObjectsUsingBlock:^(BMNewRegionModel * _Nonnull model, NSUInteger idx, BOOL * _Nonnull stop) {
                NSArray<BMNewRegionModel *> *regionArray = [self db_returnAddressSourceArrayWithParentCode:model.dcode addressType:typeArray[idx]];
...
- (void)db_addressSourceArrayWithParentCode:(NSString *)parentCode addressType:(NSString *)addressType {
    NSMutableArray<BMNewRegionModel *> *arrayM = [self db_returnAddressSourceArrayWithParentCode:parentCode addressType:addressType];
    if (self.finishedGetRegionBlock) self.finishedGetRegionBlock(arrayM);
}

- (NSMutableArray<BMNewRegionModel *> *)db_returnAddressSourceArrayWithParentCode:(NSString *)parentCode addressType:(NSString *)addressType {
